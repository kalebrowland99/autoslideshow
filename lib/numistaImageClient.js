/**
 * Numista catalogue images must load same-origin (Cloudflare + CORS on en.numista.com).
 * Server downloads bytes in /api/numista-image with browser-like headers (UA, Referer)
 * which Cloudflare lets through. Never redirect the browser directly to Numista.
 */
...
/** Safe <img src> for preview: data URLs as-is, Numista HTTPS → same-origin raw proxy. */
export function numistaDisplaySrc(url) {
  ...
  if (isNumistaCatalogueUrl(s)) return numistaImageProxyUrl(s, { mode: "raw" });
  return s;
}

/** Same-origin proxy URL for <img src> (mode=raw returns image/jpeg bytes). */

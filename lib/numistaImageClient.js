...
/** Strip grading / side labels from a Wikimedia file title before Numista search. */
export function wikimediaTitleToNumistaQuery(title) {
  return String(title || "")
    .replace(
      /\b(obverse|reverse|front|back|NGC|PCGS|ANACS|ICG|MS\d+|PR\d+|PF\d+|AU\d+|XF\d+|VF\d+|F\d+|RD|BN|RB|DMPL|proof|prooflike|edit|cropped)\b/gi,
      " ",
    )
    .replace(/[_]+/g, " ")
    .replace(/\s+/g, " ")
...
/**
 * Resolve Numista catalog title + type id from a coin name (e.g. Wikimedia file title).
 * @param {string} query
 * @param {AbortSignal | undefined} signal
 * @returns {Promise<{ title: string, typeId: number | null, issuer: string } | null>}
 */
export async function lookupNumistaCatalog(query, signal) {
  const q = wikimediaTitleToNumistaQuery(query) || String(query || "").trim();
  if (!q) return null;
  try {
    const res = await fetch("/api/numista-coins", {
      ...
      signal,
      cache: "no-store",
      body: JSON.stringify({ action: "lookup", query: q }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      if (typeof console !== "undefined" && console.warn) {
        console.warn("[numista-lookup] failed", { httpStatus: res.status, ...data });
      }
      return null;
    }
    ...
    if (typeof console !== "undefined" && console.warn) {
      console.warn("[numista-lookup] threw", e);
    }
    return null;
  }
}
...
/**
 * Wikimedia photo + Numista catalog metadata for Valcoin slideshow slots.
 * Image stays on Wikimedia (reliable); slide-up UI uses Numista title + prices.
 *
 * @param {AbortSignal | undefined} signal
 * @param {{ maxAttempts?: number, excludeSourceUrls?: Iterable<string> }} [opts]
 */
export async function fetchValcoinCoinForSlideshow(signal, { maxAttempts = 6, excludeSourceUrls } = {}) {
  const wm = await fetchRandomNumistaCoin(signal, { maxAttempts, excludeSourceUrls });
  if (!wm?.dataUrl?.startsWith("data:image/")) return null;

  const catalog = await lookupNumistaCatalog(wm.title || "", signal);
  const catalogTitle = catalog?.title?.trim() || "";
  const fallbackQuery = wikimediaTitleToNumistaQuery(wm.title) || wm.title || "";

  return {
    dataUrl: wm.dataUrl,
    title: catalogTitle || fallbackQuery,
    catalogTitle,
    wikimediaTitle: wm.title || "",
    typeId: catalog?.typeId ?? wm.typeId ?? null,

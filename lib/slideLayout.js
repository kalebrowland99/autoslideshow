...
/** Labely grocery tour or Valcoin coin tour — same slide count + scan export pipeline. */
...
/** Valcoin: opening 6-coin collage, then scan + Valcoin UI per coin (Numista photos). */
export function isValcoinCollageFormat(config) {
  return (config?.appId ?? "thrifty") === "valcoin" && (config?.outputFormat ?? "standard") === "standard";
}

/** Labely scan-tour export: scan beam on product photo, then app UI slides up. */
export function usesLabelyScanExport(config, info) {
  return (
...
/** Valcoin (both formats): scan on Numista obverse, then Valcoin UI slides up. */
export function usesValcoinScanExport(config, info) {
  if ((config?.appId ?? "thrifty") !== "valcoin" || info?.type !== "thrifty") return false;
  return isLabelyScanTourFormat(config) || isValcoinCollageFormat(config);
}

export function usesScanFrameSequenceExport(config, info) {
  return usesLabelyScanExport(config, info) || usesValcoinScanExport(config, info);
}

const VALCOIN_OUTPUT_FORMATS = new Set(["standard", "labelyScan"]);

/** @param {string | undefined} fmt */
export function normalizeValcoinOutputFormat(fmt) {
  if (fmt != null && VALCOIN_OUTPUT_FORMATS.has(fmt)) return fmt;
  return "standard";
}
...
export function getTotalSlides(config) {
  ...
  if (isValcoinCollageFormat(config)) return 1 + n;
...
export function getSlideInfo(config, slideIndex) {
  ...
  if (isLabelyScanTourFormat(config)) {
    ...
    const aid = config?.appId ?? "thrifty";
    // Valcoin reuses scan animation + shelf intro; app cards use the normal Valcoin slide renderer.
    if (aid === "valcoin") {
  ...
  if (isValcoinCollageFormat(config)) {
    ...
    return { type: "thrifty", slot: slots[itemIndex] ?? slots[0], itemIndex };
  }

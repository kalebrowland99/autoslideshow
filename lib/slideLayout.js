...
export const LABELY_SCAN_TOUR_OUTRO_SLIDES = 1;
...
export function getTotalSlides(config) {
  ...
  const f = config.outputFormat ?? "standard";
  if (isLabelyScanTourFormat(config)) return LABELY_SCAN_TOUR_INTRO_SLIDES + LABELY_SCAN_TOUR_SLOTS + LABELY_SCAN_TOUR_OUTRO_SLIDES;
...
export function getSlideInfo(config, slideIndex) {
  ...
  if (isLabelyScanTourFormat(config)) {
    ...
    const labelyOutroIndex = LABELY_SCAN_TOUR_INTRO_SLIDES + LABELY_SCAN_TOUR_SLOTS;
    if (slideIndex === labelyOutroIndex) {
      return { type: "labelyOutro" };
    }
...
export function slideIndexToSlotIndex(slideIndex, config) {
  ...
  if (isLabelyScanTourFormat(config)) {
    const labelyOutroIndex = LABELY_SCAN_TOUR_INTRO_SLIDES + LABELY_SCAN_TOUR_SLOTS;
    if (slideIndex === 0 || slideIndex === labelyOutroIndex) return null;

...
/** Duration + density tuned like SwiftUI-style center bursts (web export can't host ConfettiSwiftUI). */
const CONFETTI_SECONDS = 1.5;
const CONFETTI_PARTICLE_COUNT = 150;

function createConfettiParticles(seed, count, w, h) {
  const particles = [];
  /** Screen midpoint explosion — radial symmetry similar to simibac/ConfettiSwiftUI-style bursts */
  const cx = w / 2 + (hashUnit(seed, 901) - 0.5) * 16;
  const cy = h / 2 + (hashUnit(seed, 902) - 0.5) * 16;
  for (let i = 0; i < count; i++) {
    const angle = hashUnit(seed, i * 7 + 1) * Math.PI * 2;
    const speed = 12 + hashUnit(seed, i * 7 + 3) * 26;
    const vx = Math.cos(angle) * speed;
    const vy = Math.sin(angle) * speed;
    const pw = 5 + hashUnit(seed, i * 7 + 4) * 11;
    const ph = 3 + hashUnit(seed, i * 7 + 5) * 10;
    const hue = Math.floor(hashUnit(seed, i * 7 + 6) * 330);
    const lit = 50 + hashUnit(seed, i * 7 + 8) * 22;
    const jitter = 22 * hashUnit(seed, i * 7 + 9);
    const jitterA = hashUnit(seed, i * 7 + 10) * Math.PI * 2;
    particles.push({
      x: cx + Math.cos(jitterA) * jitter,
      y: cy + Math.sin(jitterA) * jitter,
      vx,
      vy,
      g: 0.42 + hashUnit(seed, i * 7 + 11) * 0.32,
      drag: 0.984 + hashUnit(seed, i * 7 + 12) * 0.01,
      rot: hashUnit(seed, i * 7 + 13) * Math.PI * 2,
      vr: (hashUnit(seed, i * 7 + 14) - 0.5) * 0.55,
      w: pw,
      h: ph,
      color: `hsl(${hue}, 90%, ${lit}%)`,
      shape: hashUnit(seed, i * 7 + 15), // 0–1 → rect | circle | strip
    });
  }
  return particles;
}
...
function drawConfettiLayer(ctx, particles, fade01) {
  const a = Math.max(0, Math.min(1, fade01)) * 0.94;
  for (const p of particles) {
    ctx.save();
    ctx.globalAlpha = a;
    ...
    if (sh > 0.78) {
      ctx.beginPath();
      ctx.arc(0, 0, Math.max(p.w, p.h) * 0.38, 0, Math.PI * 2);
      ctx.fill();
    } else if (sh > 0.42) {
      const ww = p.w * 1.15;
      const hh = p.h * 0.65;
...
export async function buildLabelyScanFrameSequence({
  ...
  const confettiSeed = (imageVariationSeed | 0) ^ 0x30ff371;
  const particles = createConfettiParticles(confettiSeed, CONFETTI_PARTICLE_COUNT, OUT_W, OUT_H);
  const confettiFrames = Math.min(holdFrames, Math.ceil(CONFETTI_SECONDS * fps));

  for (let h = 0; h < holdFrames; h++) {
    if (h < confettiFrames) {
      stepConfetti(particles);
      const fadeOut = 1 - h / Math.max(1, confettiFrames - 1);
      const burstIn = Math.min(1, (h + 8) / 14);
      const fade01 = burstIn * (0.35 + 0.65 * fadeOut);

      const c = document.createElement("canvas");
      c.width = OUT_W;
      c.height = OUT_H;
      const ctx = c.getContext("2d");
      ctx.drawImage(labelyCanvas, 0, 0);
      drawConfettiLayer(ctx, particles, fade01);
      out.push(c);
    } else {

...
function proxyRemoteImageSrc(src) {
  ...
  try {
    ...
    if (host.includes("numista.com")) {
      return `/api/numista-image?${new URLSearchParams({ url: s, mode: "redirect" }).toString()}`;
    }
  } catch {
    /* ignore */
  }
...
function loadRasterImage(src) {
  return new Promise((resolve) => {
    ...
    im.src = proxyRemoteImageSrc(src);
  });
}

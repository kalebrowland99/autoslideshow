...
async function loadRasterFromProxy(remoteUrl) {
  const proxy = numistaImageProxyUrl(remoteUrl);
  if (proxy === remoteUrl) return null;
  try {
    ...
    try {
      const im = await loadImageElement(url, { crossOrigin: false });
      return im;
    } finally {
...
async function loadRasterImage(src) {
  ...
  if (s.startsWith("data:")) {
    return loadImageElement(s, { crossOrigin: false });
  }
  if (isNumistaCatalogueUrl(s)) {
    const fromProxy = await loadRasterFromProxy(s);
...
export async function buildLabelyScanFrameSequence({
  ...
  function drawProductBackground(ctx) {
    ...
    if (productImg && productImg.width) {
      ctx.save();
      ctx.translate(box.x, box.y);
      drawContain(ctx, productImg, box.w, box.h, imageVariationSeed);
      ctx.restore();
    } else {
...
/**
 * Full-screen intro hold (scan tour slide 0) — canvas only, no DOM capture.
...
export async function captureShelfIntroCanvas(productDataUrl, imageVariationSeed = 0) {
  ...
  const productImg = await loadRasterImage(productDataUrl?.trim?.() ?? "");
  if (productImg?.width) {
    const maxW = Math.round(OUT_W * 0.88);
    const maxH = Math.round(OUT_H * 0.68);
    const dx = (OUT_W - maxW) / 2;
    const dy = (OUT_H - maxH) / 2;
    ctx.save();
    ctx.translate(dx, dy);
    drawContain(ctx, productImg, maxW, maxH, imageVariationSeed);
    ctx.restore();
  } else {
    drawPlaceholderBg(ctx, OUT_W, OUT_H);
  }

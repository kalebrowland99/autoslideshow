...
const CONFETTI_SECONDS = 1.35;
const CONFETTI_PARTICLE_COUNT = 96;

function createConfettiParticles(seed, count, w, h) {
  const particles = [];
  const ox = w * (0.38 + hashUnit(seed, 901) * 0.24);
  const oy = h * (0.22 + hashUnit(seed, 902) * 0.14);
  for (let i = 0; i < count; i++) {
    const dir = hashUnit(seed, i * 7 + 1) * Math.PI * 2;
    const spread = hashUnit(seed, i * 7 + 2);
    const speed = 9 + hashUnit(seed, i * 7 + 3) * 16;
    const angle = dir + (spread - 0.5) * 1.1;
    const pw = 5 + hashUnit(seed, i * 7 + 4) * 11;
    const ph = 3 + hashUnit(seed, i * 7 + 5) * 9;
    const hue = Math.floor(hashUnit(seed, i * 7 + 6) * 330);
    const lit = 52 + hashUnit(seed, i * 7 + 8) * 18;
    particles.push({
      x: ox + (hashUnit(seed, i * 7 + 9) - 0.5) * 28,
      y: oy + (hashUnit(seed, i * 7 + 10) - 0.5) * 18,
      vx: Math.cos(angle) * speed,
      vy: Math.sin(angle) * speed - 7,
      g: 0.38 + hashUnit(seed, i * 7 + 11) * 0.28,
      drag: 0.986 + hashUnit(seed, i * 7 + 12) * 0.008,
      rot: hashUnit(seed, i * 7 + 13) * Math.PI * 2,
      vr: (hashUnit(seed, i * 7 + 14) - 0.5) * 0.45,
      w: pw,
      h: ph,
      color: `hsl(${hue}, 88%, ${lit}%)`,
      round: hashUnit(seed, i * 7 + 15) > 0.72,
    });
  }
  return particles;
}
...
function drawConfettiLayer(ctx, particles, fade01) {
  ...
  for (const p of particles) {
    ...
    if (p.round) {
      ctx.beginPath();
      ctx.arc(0, 0, Math.max(p.w, p.h) * 0.35, 0, Math.PI * 2);
      ctx.fill();
    } else {

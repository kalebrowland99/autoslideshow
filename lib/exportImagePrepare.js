...
const WSRV_HOST = /^https:\/\/wsrv\.nl\//i;
...
async function fetchWsrvAsDataUrl(wsrvUrl) {
  try {
    const res = await fetch(wsrvUrl, { mode: "cors", credentials: "omit" });
    if (!res.ok) return "";
...
function buildWsrvUrl(remoteUrl) {
  const stripped = remoteUrl.replace(/^https?:\/\//i, "");
  return `https://wsrv.nl/?url=${encodeURIComponent(stripped)}`;
}

async function wsrvDataUrl(remoteUrl) {
  return await fetchWsrvAsDataUrl(buildWsrvUrl(remoteUrl));
}
...
/**
 * Fetch image bytes via wsrv.nl (CORS=*) — falls back to same-origin /api/numista-image.
 * Server-side numista fetches from Vercel often get blocked by Cloudflare;
 * wsrv has its own IPs and is purpose-built for hot-linking.
...
export async function remoteImageToDataUrl(remoteUrl) {
  ...
  if (WSRV_HOST.test(u)) {
    return await fetchWsrvAsDataUrl(u);
  }

  if (NUMISTA_HOST.test(u)) {
    const viaWsrv = await wsrvDataUrl(u);
    if (viaWsrv.startsWith("data:image/")) return viaWsrv;

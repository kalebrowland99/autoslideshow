/**
 * Export must never fetch cross-origin images from the browser (CORS).
 * Server proxies bytes; we rewrite slot data + live DOM <img> src to data: URLs
 * before html-to-image runs (it only skips fetch when src is already data:).
 */
...
async function readResponseAsDataUrl(res) {
...
/**
 * Fetch image bytes via same-origin API (server downloads remote host).
...
export async function remoteImageToDataUrl(remoteUrl) {
  ...
  const apiPath = NUMISTA_HOST.test(u) ? "/api/numista-image" : "/api/export-image";
  ...
  try {
    ...
    if (res.ok && fromJson.startsWith("data:image/")) return fromJson;
  } catch {
    /* raw fallback */
  }

  try {
    const res = await fetch(`${apiPath}?${new URLSearchParams({ url: u, mode: "raw" })}`);
    if (!res.ok) return "";
    return await readResponseAsDataUrl(res);
  } catch {
    return "";
  }
}
...
export async function inlineRemoteImagesInElement(root, { strict = false } = {}) {
  ...
  if (strict && failures.length > 0) {
    throw new Error(
      `Could not prepare ${failures.length} image(s) for export (server proxy failed). Try regenerating slides.`,
    );
  }
}

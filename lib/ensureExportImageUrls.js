  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    for (const key of IMAGE_KEYS) {
      ...
      if (dataUrl.startsWith("data:image/")) {
        next[key] = dataUrl;
      } else if (isNumistaCatalogueUrl(url)) {
        const proxy = numistaImageProxyUrl(url);
        if (proxy) next[key] = proxy;
        else console.warn("[export] Could not inline Numista image for slot", i, key);
      }
    }

...
export function mergePersistedConfig(defaults, emptySlotFn, saved) {
  ...
  if (saved.labelyUseGoogleImages && !saved.labelyUseBraveImages) {
    merged.labelyUseBraveImages = true;
  }
  if (saved.labelyUseBingImages && !saved.labelyUseBraveImages) {
    merged.labelyUseBraveImages = true;
  }
  delete merged.labelyUseGoogleImages;
  delete merged.labelyUseBingImages;
  return merged;
}
